...
public class ExpectJGivenSpread {
	int NN = 500;
	int TT = 40;
	int KK = 81;
	int LL = 10000;
	...
	public static void main(String[] args) {
		...
		ExpectJGivenSpread pu = new ExpectJGivenSpread();
	...
	void process(String inputfile, String tradefile, int limit, int vbin, int vlen) {
		...
		outputExpect("ExpectJGivenSpread_" + "VB" + this.VBIN + "VL" + this.VLEN);
	}
	...
	void processX(StrOrder p1, StrOrder p2) {
		...
		if (p1.time / 1000 - p2.time / 1000 == 1 && hm.containsKey(p2.time / 1000)) {
			...
			// ld1.add(t1);
			// ld2.add(t0 - t1);
			int index = ((int) ((Math.abs(Math.log(p2.t1 / p2.mid)) + Math.abs(Math.log(p2.t2 / p2.mid))) * 1e5
					/ VBIN));
			if (index >= 0 && index < VLEN) {
				ld.get(index).t1++;
				ld.get(index).t2 += t0 - t1;
			}
