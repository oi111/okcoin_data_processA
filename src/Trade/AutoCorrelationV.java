		...
		for (int i = 0; i < LEN; i += BIN) {
			...
			for (int j = LEN; j < lt.size(); j++)
				if (lt.get(j).t1 / 1000 - lt.get(j - i).t1 / 1000 <= i * (1 + 0.05)) {
					if (this.FLG == 0)
						tc.add(lt.get(j).t3, lt.get(j - i).t3);
					else
						tc.add(Math.abs(lt.get(j).t3), Math.abs(lt.get(j - i).t3));
					num++;
				}
