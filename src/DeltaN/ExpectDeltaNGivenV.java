...
public class ExpectDeltaNGivenV {
	int NN = 500;
	int TT = 40;
	int KK = 81;
	int LL = 10000;
	...
	public static void main(String[] args) {
		...
		ExpectDeltaNGivenV pu = new ExpectDeltaNGivenV();
	...
	void process(String inputfile, int limit, int bin, int cc, int ff, int tt, int vlen, int flg) {
		...
		input.closeFile();
		outputNumber("ExpectDeltaNGivenV_" + "B" + BIN + "C" + CC + "F" + FF + "T" + TT + "VL" + VLEN + "FLG" + FLG);
		outputExpect("ExpectDeltaNGivenV_" + "B" + BIN + "C" + CC + "F" + FF + "T" + TT + "VL" + VLEN + "FLG" + FLG);
	...
	void processX(StrOrder p1, StrOrder p2) {
		...
		for (int i = 0; i < gg.length; i++) {
			...
			v = (int) (v / (gg[i] + 0.0) * 10 / BIN + VLEN / 2);
			if (p1.time / 1000 - time[(index + TT - gg[i]) % TT] / 1000 <= gg[i] * (1 + 0.05) && v >= 0 && v < VLEN) {
				for (int j = 0; j < pt.length; j += CC) {
					if (FLG == 0 && sum[i][j] == 0)
						continue;
					tltk.get(j / CC).get(v).t2 += (double) (sum[i][j] / 1000.0);
					tltk.get(j / CC).get(v).t1++;
				}
			}
		}
	...
	void outputNumber(String outputfile) {
		...
		for (int i = 0; i < tltk.size(); i++) {
			for (int j = 0; j < tltk.get(i).size(); j++) {
				output.write(tltk.get(i).get(j).t1 + " ");
			}
			output.write("\n");
		}
		output.closeFile();
