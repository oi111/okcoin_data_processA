...
public class ExpectDeltaNGivenNAndS {
	int NN = 600;
	int TT = 40;
	int KK = 100;
	int LL = 10000;
	...
	List<List<List<ITK>>> ld3 = new ArrayList<List<List<ITK>>>();

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		ExpectDeltaNGivenNAndS pu = new ExpectDeltaNGivenNAndS();
	...
	void process(String inputfile, int limit, int binsize, int cc, int kk, int vbin, int vlen, int flg) {
		...
		input.closeFile();
		outputExpect("ExpectDeltaNGivenNAndS_B" + this.BINSIZE + "C" + this.CC + "K" + this.KK + "VB" + VBIN + "VL"
				+ VLEN + "FLG" + FLG);
		outputNumber("ExpectDeltaNGivenNAndS_B" + this.BINSIZE + "C" + this.CC + "K" + this.KK + "VB" + VBIN + "VL"
				+ VLEN + "FLG" + FLG);
		// WZ: without zero;
	}
	...
	void processX(StrOrder p1, StrOrder p2) {
		...
		int v = (p1.mid - p2.mid) / VBIN + VLEN / 2;
		if (p1.time / 1000 - p2.time / 1000 == 1 && v >= 0 && v < VLEN) {
			for (int i = 0; i < pt.length; i += CC)
				if (pt[i].t2 / BINSIZE < KK) {
					...
					ld3.get(i / CC).get(v).get(pt[i].t2 / BINSIZE).t1 += a[index][i];
		...
		for (int i = 0; i < ld3.size(); i++) {
			for (int j = 0; j < ld3.get(i).size(); j++) {
				for (int k = 0; k < ld3.get(i).get(j).size(); k++)
					output.write(ld3.get(i).get(j).get(k).t1 / (ld3.get(i).get(j).get(k).t2 + 1e-8) + " ");
				output.write("\n");
			}
		}
		output.closeFile();
	...
	void outputNumber(String outputfile) {
		...
		for (int i = 0; i < ld3.size(); i++) {
			for (int j = 0; j < ld3.get(i).size(); j++) {
				for (int k = 0; k < ld3.get(i).get(j).size(); k++)
					output.write((ld3.get(i).get(j).get(k).t2) + " ");
				output.write("\n");
			}
		}
		output.closeFile();
