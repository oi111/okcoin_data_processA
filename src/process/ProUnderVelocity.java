...
public class ProUnderVelocity {
	...
	public static void main(String[] args) {
		...
		ProUnderVelocity pu = new ProUnderVelocity();
	...
	void process(String inputfile, int limit, int tt, int ll, int bin) {
		...
		while ((line = input.read()) != null) {
			addOne(line);
			processX(current, ord[n % this.TT]);
			ord[n % this.TT] = current;
			if ((++n) % 10000 == 0)
		...
		input.closeFile();
		outputData("ProUnderVelocity_" + TT + "_" + LL + "_" + BIN);
		outputPro("ProUnderVelocity_" + TT + "_" + LL + "_" + BIN);
	}
	...
	void processX(StrOrder p1, StrOrder p2) {
		...
		index = (index + 1) % TT;
		int v = (p1.mid - p2.mid) / BIN;
		time[index] = (int) (p1.time / 1000);
		for (int i = 0; i < pt.length; i++) {
			a[index][i] = pt[i];
		}
		if (v + KK / 2 >= 0 && v + KK / 2 < KK && p1.time / 1000 - p2.time / 1000 <= TT * (1 + 0.05)) {
			for (int i = 0; i < pt.length; i += CC)
				if (ld3.get(v + KK / 2).get(i / CC).size() < LL && a[index][i] != 0)
					ld3.get(v + KK / 2).get(i / CC).add((double) (a[index][i] / 1000.0));
		}
	}
