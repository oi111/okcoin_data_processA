...
public class ProUnderVelocityGivenTimeALL {
	int NN = 500;
	int TT = 40;
	int KK = 81;
	int LL = 10000;
	...
	public static void main(String[] args) {
		...
		ProUnderVelocityGivenTimeALL pu = new ProUnderVelocityGivenTimeALL();
	...
	void process(String inputfile, int limit, int tt, int ll) {
		this.TT = tt;
		this.LL = ll;
		init();
		...
		input.closeFile();
		outputData("ProUnderVelocityGivenTimeALL_" + TT + "_" + LL);
		outputPro("ProUnderVelocityGivenTimeALL_" + TT + "_" + LL);
	}
	...
	void processX(StrOrder p1, StrOrder p2) {
		...
		ak1++;
		if (p1.time / 1000 - time[index] / 1000 <= TT * (1 + 0.05)) {
