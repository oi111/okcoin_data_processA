...
public class ProUnderVelocityGivenV {
	int NN = 500;
	int TT = 40;
	int KK = 81;
	int LL = 10000;
	...
	public static void main(String[] args) {
		...
		ProUnderVelocityGivenV pu = new ProUnderVelocityGivenV();
	...
	void process(String inputfile, int limit, int tt, double givenv, int binsize, int cc) {
		this.TT = tt;
		this.GIVENV = givenv;
		...
		input.closeFile();
		outputData("ProUnderVelocityGivenV_T" + TT + "G" + (int) (GIVENV * 10) + "F" + FF + "B" + BINSIZE + "C" + CC);
		outputPro("ProUnderVelocityGivenV_T" + TT + "G" + (int) (GIVENV * 10) + "F" + FF + "B" + BINSIZE + "C" + CC);
	}
	...
	void processX(StrOrder p1, StrOrder p2) {
		...
		for (int i = 0; i < gg.length; i++) {
			...
			if (v + BINSIZE >= GIVENV * gg[i] && v - BINSIZE <= GIVENV * gg[i]
