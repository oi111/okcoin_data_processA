...
public class ProUnderVelocityGivenV {
	...
	void process(String inputfile, int limit, int bin, int cc, int ff, int tt, int v, String other) {
		...
		input.closeFile();
		outputTime("ProUnderVelocityGivenV_" + "B" + BIN + "C" + CC + "F" + FF + "T" + TT + "V" + V + "OT" + OTHER);
		outputData("ProUnderVelocityGivenV_" + "B" + BIN + "C" + CC + "F" + FF + "T" + TT + "V" + V + "OT" + OTHER);
		outputPro("ProUnderVelocityGivenV_" + "B" + BIN + "C" + CC + "F" + FF + "T" + TT + "V" + V + "OT" + OTHER);
	}
	...
	void processX(StrOrder p1, StrOrder p2) {
		...
		for (int i = 0; i < gg.length; i++) {
			...
			double v = ((p1.mid - mid[(index + TT - gg[i]) % TT]) / 2.0 * 10.0 / (gg[i]) / 1000.0);
