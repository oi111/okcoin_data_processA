...
public class ProDeltaNFlg {
	...
	public static void main(String[] args) {
		...
		ProDeltaNFlg pu = new ProDeltaNFlg();
	...
	void process(String inputfile, int limit, int ff) {
		this.FF = ff;
		init();
		...
		input.closeFile();
		outputData("ProDeltaNFlg_" + LOGLEN + "_" + FF);
	}
	...
	void processX(StrOrder p1, StrOrder p2) {
		...
		if (p1.time / 1000 - p2.time / 1000 == 1) {
			for (int i = 0; i < pt.length; i += CC)
				if (pt[i].t2 / FF < KK) {
					if (a[index][i] < 0)
						ld3.get(i / CC).get(pt[i].t2 / FF).t1++;
					if (a[index][i] == 0)
						ld3.get(i / CC).get(pt[i].t2 / FF).t2++;
					if (a[index][i] > 0)
						ld3.get(i / CC).get(pt[i].t2 / FF).t3++;
				}
		}
	}
